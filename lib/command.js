...
class CommonBin {
   ...
   * @param {Object} context.argv - argv parse result by yargs, `{ _: [ 'start' ], '$0': '/usr/local/bin/nodinx-cli', baseDir: 'simple'}`
  ...
  add(name, target) {
    ...
    if (!(target.prototype instanceof CommonBin)) {
      ...
      assert(target.prototype instanceof CommonBin,
        'command class should be sub class of nodinx-cli');
    }
  ...
  errorHandler(err) {
    ...
    console.error('⚠️  Command Error, enable `DEBUG=nodinx-cli` for detail'.red);
